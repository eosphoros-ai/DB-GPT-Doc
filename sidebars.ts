import type { SidebarsConfig } from "@docusaurus/plugin-content-docs";
const sidebars: SidebarsConfig = {
  docs: [
    {
      type: "doc",
      id: "overview",
      label: "Overview",
    },

    {
      type: "doc",
      id: "quickstart",
      label: "Quickstart",
    },

    {
      type: "category",
      label: "Installation",
      collapsed: false,
      collapsible: false,
      items: [
        {
          type: "doc",
          id: "installation/sourcecode",
        },
        {
          type: "doc",
          id: "installation/docker",
        },
        {
          type: "doc",
          id: "installation/docker_compose",
        },
        {
          type: "category",
          label: "Model Service Deployment",
          items: [
            {
              type: "doc",
              id: "installation/model_service/stand_alone",
            },
            {
              type: "doc",
              id: "installation/model_service/cluster",
            },
          ],
        },
        {
          type: "category",
          label: "Adanced Usage",
          items: [
            {
              type: "doc",
              id: "installation/advanced_usage/vLLM_inference",
            },
            {
              type: "doc",
              id: "installation/advanced_usage/OpenAI_SDK_call",
            },
          ],
        },
      ],
      link: {
        type: "generated-index",
        description:
          "DB-GPT provides a wealth of installation and deployment options, supporting source code deployment, Docker deployment, cluster deployment and other modes. At the same time, it can also be deployed and installed based on the AutoDL image.",
        slug: "installation",
      },
    },
    {
      type: "category",
      label: "Operation Manual",
      collapsed: false,
      collapsible: false,
      items: [
        {
          type: "category",
          label: "Getting Started Tutorial",
          items: [
            {
              type: "doc",
              id: "operation_manual/started_tutorial/chat_knowledge",
            },
            {
              type: "doc",
              id: "operation_manual/started_tutorial/chat_data",
            },
            {
              type: "doc",
              id: "operation_manual/started_tutorial/chat_excel",
            },
            {
              type: "doc",
              id: "operation_manual/started_tutorial/chat_db",
            },
            {
              type: "doc",
              id: "operation_manual/started_tutorial/chat_dashboard",
            },
            {
              type: "doc",
              id: "operation_manual/started_tutorial/agent",
            },
          ],
        },
      ],
      link: {
        type: "generated-index",
        description: "DB-GPT Operation Manual",
        slug: "operation_manual",
      },
    },

    {
      type: "category",
      label: "Modules",
      collapsed: true,
      items: [
        {
          type: "category",
          label: "Service-oriented Multi-model Management",
          collapsed: true,
          items: [{ type: "autogenerated", dirName: "smmf" }],
          link: { type: "doc", id: "modules/smmf/index" },
        },
      ],
    },

    {
      type: "doc",
      label: "Application",
      id: "application/index",
    },

    {
      type: "doc",
      id: "changelog",
      label: "ChangeLog",
    },

    {
      type: "category",
      label: "Example",
      collapsed: true,
      collapsible: false,
      items: [{ type: "autogenerated", dirName: "example" }],
      link: {
        type: "generated-index",
        description: "example",
        slug: "example",
      },
    },
  ],
  use_case: [
    {
      type: "doc",
      id: "use_cases/sql_generation_and_diagnosis",
      label: "sql_generation_and_diagnosis",
    },
    {
      type: "doc",
      id: "use_cases/tool_use_with_plugin",
      label: "tool_use_with_plugin",
    },
  ],
};

export default sidebars;
